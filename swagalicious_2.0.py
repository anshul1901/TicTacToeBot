...
class Swagalicious():
	def __init__(self):
		self.bs_score_matrix = [
			[16, 24, 24, 16],
			[24, 32, 32, 24],
			[24, 32, 32, 24],
			[16, 24, 24, 16]
		]
		self.opp_bs_score_matrix = [
			[16, 24, 24, 16],
			[24, 32, 32, 24],
			[24, 32, 32, 24],
			[16, 24, 24, 16]
	...
	def move(self, board, old_move, flag):
		...
		if can_win == 1:
			...
			temp_matrix = [[6, 4, 4, 6], [4, 3, 3, 4], [4, 3, 3, 4], [6, 4, 4, 6]]

		for i in range(x*4, x*4+3):
			for j in range(y*4, y*4+3):
				# try:
				if bds[i][j] == flag:
					temp_matrix[i % 4][j % 4] *= 1
				elif bds[i][j] == '-':
					temp_matrix[i % 4][j % 4] *= 0.5
				else:
					temp_matrix[i % 4][j % 4] *= 0
				# except:
				# 	print i, j
		temp_bs_score = 0
		for i in range(4):
			...
			product_coloumn = 1
			for j in range(4):
				product_row *= temp_matrix[i][j]
				product_coloumn *= temp_matrix[j][i]
			temp_bs_score += product_row + product_coloumn

		temp_bs_score += temp_matrix[0][1]*temp_matrix[1][0]*temp_matrix[1][2]*temp_matrix[0][2]
		temp_bs_score += temp_matrix[1][1]*temp_matrix[2][0]*temp_matrix[2][2]*temp_matrix[1][2]
		temp_bs_score += temp_matrix[0][2]*temp_matrix[1][1]*temp_matrix[1][3]*temp_matrix[0][3]
		temp_bs_score += temp_matrix[1][2]*temp_matrix[2][1]*temp_matrix[2][3]*temp_matrix[1][3]

		self.bs_score_matrix[x][y] = temp_bs_score
		return bm
	...
	def temp_h(self, board, depth, isMax, flag, old_move):
		cells = board.find_valid_move_cells(old_move)
		maxxx = 100000
		temp = 0
		for cell in cells:
	...
	def heuristic(self, board, depth, isMax, flag, old_move):
		...
		for i in range(x*4, x*4+3):
			for j in range(y*4, y*4+3):
				try:
					if bds[i][j] == flag:
						temp_matrix[i % 4][j % 4] *= 1
						opp_temp_matrix[i % 4][j % 4] *= 0
					elif bds[i][j] == '-':
						temp_matrix[i % 4][j % 4] *= 0.5
						opp_temp_matrix[i % 4][j % 4] *= 0.5
					else:
						temp_matrix[i % 4][j % 4] *= 0
						opp_temp_matrix[i % 4][j % 4] *= 1
				except:
					print i, j
		# print temp_matrix
		temp_bs_score = 0
		...
		for i in range(4):
			...
			opp_product_row = 1
			product_coloumn = 1
			opp_product_coloumn = 1
			for j in range(4):
				product_row *= temp_matrix[i][j]
				opp_product_row *= temp_matrix[i][j]
				product_coloumn *= temp_matrix[j][i]
				opp_product_coloumn *= temp_matrix[j][i]
			temp_bs_score += product_row + product_coloumn
			opp_temp_bs_score += opp_product_row + opp_product_coloumn
		...
		opp_temp_bs_score += opp_temp_matrix[0][1]*opp_temp_matrix[1][0]*opp_temp_matrix[1][2]*opp_temp_matrix[0][2]
		opp_temp_bs_score += opp_temp_matrix[1][1]*opp_temp_matrix[2][0]*opp_temp_matrix[2][2]*opp_temp_matrix[1][2]
		opp_temp_bs_score += opp_temp_matrix[0][2]*opp_temp_matrix[1][1]*opp_temp_matrix[1][3]*opp_temp_matrix[0][3]
		opp_temp_bs_score += opp_temp_matrix[1][2]*opp_temp_matrix[2][1]*opp_temp_matrix[2][3]*opp_temp_matrix[1][3]

		self.bs_score_matrix[x][y] = temp_bs_score
		self.opp_bs_score_matrix[x][y] = opp_temp_bs_score
		# print temp_bs_score
		final_score = 0
		opp_final_score = 0
		for i in range(4):
			...
			opp_product_coloumn = 1
			for j in range(4):
				product_row *= self.bs_score_matrix[i][j]
				product_coloumn *= self.bs_score_matrix[j][i]
				opp_product_row *= self.opp_bs_score_matrix[i][j]
				opp_product_coloumn *= self.opp_bs_score_matrix[j][i]
			final_score += product_row + product_coloumn
			opp_final_score += opp_product_row + opp_product_coloumn

		final_score += self.bs_score_matrix[0][1]*self.bs_score_matrix[1][0]*self.bs_score_matrix[1][2]*self.bs_score_matrix[0][2]
		final_score += self.bs_score_matrix[1][1]*self.bs_score_matrix[2][0]*self.bs_score_matrix[2][2]*self.bs_score_matrix[1][2]
		final_score += self.bs_score_matrix[0][2]*self.bs_score_matrix[1][1]*self.bs_score_matrix[1][3]*self.bs_score_matrix[0][3]
		final_score += self.bs_score_matrix[1][2]*self.bs_score_matrix[2][1]*self.bs_score_matrix[2][3]*self.bs_score_matrix[1][3]

		opp_final_score += self.opp_bs_score_matrix[0][1]*self.opp_bs_score_matrix[1][0]*self.opp_bs_score_matrix[1][2]*self.opp_bs_score_matrix[0][2]
		opp_final_score += self.opp_bs_score_matrix[1][1]*self.opp_bs_score_matrix[2][0]*self.opp_bs_score_matrix[2][2]*self.opp_bs_score_matrix[1][2]
		opp_final_score += self.opp_bs_score_matrix[0][2]*self.opp_bs_score_matrix[1][1]*self.opp_bs_score_matrix[1][3]*self.opp_bs_score_matrix[0][3]
		opp_final_score += self.opp_bs_score_matrix[1][2]*self.opp_bs_score_matrix[2][1]*self.opp_bs_score_matrix[2][3]*self.opp_bs_score_matrix[1][3]
		# print self.bs_score_matrix
		# print final_score
		# print final_score, opp_final_score
		return (final_score - opp_final_score)/1000000000
